...
for stock in stock_list:
    ...
    # Feature Engineering to avoid flat forecasts
    ...
    features = ['Lag_1', 'Returns', 'Volatility', 'MA_50', 'MA_200']
    df[features] = df[features].fillna(0)

    train_size = int(len(df) * 0.8)
    train, test = df.iloc[:train_size], df.iloc[train_size:]

    # Models with dynamic features
    ...
    future_xgb = [xgb_model.predict(last_row)[0] for _ in range(forecast_days)]

    rf_model = RandomForestRegressor(n_estimators=100)
    rf_model.fit(train[features], train['Close'])
    rf_pred = rf_model.predict(test[features])
    future_rf = [rf_model.predict(last_row)[0] for _ in range(forecast_days)]
